...
class SVMlight(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        random_code = '-'.join(str(local_random.randint(0,1000000)) for _ in range(5)) #this would allow to run parallel instances of predict

        self.model = join(self.tmpdir.name, 'model-'+random_code)
        traindata_path = join(self.tmpdir.name, f'train-{random_code}-.dat')
        transductions_path = join(self.tmpdir.name, f'transd_labels-{random_code}.dat')
